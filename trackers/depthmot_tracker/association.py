...
def calculate_depth_similarity(stats1, stats2, max_boost=2.0):
    simil = 1 - cdist(stats1, stats2, metric='cosine')

    # Check if any values in simil are NaN
    if np.isnan(simil).any():
        # Return a zero matrix with the same shape
        return np.zeros_like(simil)
...
def mask_iou_batch(masks1, masks2):
    """
    ...
    masks1 = np.expand_dims(masks1, 1)  # Shape: (N, 1, H, W)
    masks2 = np.expand_dims(masks2, 0)  # Shape: (1, M, H, W)

    masks1 = masks1.astype(np.uint8)  # This will convert to binary (0 or 1)
    masks2 = masks2.astype(np.uint8)

    # Compute the intersection and union for each pair of masks
    intersection = np.logical_and(masks1, masks2).sum(axis=(2, 3))
    union = np.logical_or(masks1, masks2).sum(axis=(2, 3))
...
def associate(
    ...
    iou_matrix = iou_batch(detections, trackers)
    # if mask_embs is not None and trk_mask is not None:
    #     exp_mask_iou = np.exp(mask_iou_batch(mask_embs, trk_mask))
    #     # Step 2: Multiply iou_batch with exp(mask_iou_batch) element-wise
    #     iou_matrix = iou_matrix * exp_mask_iou
    ...
    if not emb_off:
        if grid_off:
            ...
            # depth_cost = calculate_depth_similarity(depths_embs, trk_depth) if (trk_depth is not None and depths_embs is not None) else None
            depth_cost = calculate_depth_similarity(mask_embs, trk_mask) if (
                        trk_mask is not None and mask_embs is not None) else None
        else:
    ...
    if min(iou_matrix.shape) > 0:
        ...
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            ...
            final_cost = -(iou_matrix + angle_diff_cost + emb_cost + depth_cost)
