...
def get_mot_loader(dataset, test, data_dir="datasets", workers=4, size=(800, 1440)):
    load_dotenv()
    data_dir = os.getenv("DATASET_PATH", "datasets")  # fallback to "datasets" if not set
...
class MOTDataset(torch.utils.data.Dataset):
    ...
    def pull_item(self, index):
        ...
        try:
            res1, img_info1, file_name1 = self.annotations[index-1]
            prefix1 = file_name.split('/')[0]
            prefix2 = file_name1.split('/')[0]
            if prefix1 == prefix2:
        ...
        return [img_file, img_file1], img, res.copy(), img_info, np.array([id_])
    ...
    def __getitem__(self, index):
        ...
        """
        img_path, img, target, img_info, img_id = self.pull_item(index)
        tensor, target = self.preproc(img, target, self.input_dim)
        return (tensor, img), target, img_info, img_id, img_path
